...
use state::{get_primary_key_field, methods_singleton, singleton};
use types::Type;
...
fn add_filter_arguments(filter: FilterExpression, args: &mut Args, table_name: &str) {
    match filter {
        FilterExpression::Filter(filter) => {
            match filter.operand1 {
                RValue::Identifier(identifier) => {
                    let tables = singleton();
                    if let Some(field_type) = tables.get(table_name).and_then(|table| table.get(&identifier)) {
                        add(args, Some(identifier), field_type.node.clone(), filter.operand2, table_name)
                    }
                },
...
fn add_with_method(args: &mut Args, method_name: &str, object_name: &str, index: usize, expr: Expression, table_name: &str) {
    ...
    if let Some(field_type) = tables.get(table_name).and_then(|table| table.get(object_name)) {
        if let Some(&(_, ref arguments)) = methods.get(&field_type.node).and_then(|type_methods| type_methods.get(method_name)) {
            add_expr(args, Arg {
                expression: expr,
                field_name: None,
                typ: arguments[index].clone(),
            }, table_name);
        }
    }
}
