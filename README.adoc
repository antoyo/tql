...
fn main() {
    ...
    let text = String::new();
    let result = sql!(Model.insert(text = &text, date_added = Utc::now()));
    ...
    let items = sql!(TodoItem.sort(-date_added)[..10]);
}
----
...
error[E0308]: mismatched types
  --> src/main.rs:47:18
   |
47 |     let result = sql!(TodoItem.insert(text = text, date_added = Utc::now(), done = false));
   |                  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ expected &str, found struct `std::string::String`
   |
   = note: expected type `&str`
              found type `std::string::String`
   = help: try with `&sql!(TodoItem.insert(text = text, date_added = Utc::now(), done = false))`
...
error[E0308]: mismatched types
  --> examples/todo.rs:49:46
   |
49 |     let result = sql!(TodoItem.insert(text = text, date_added = Utc::now(), done = false));
   |                                              ^^^^
   |                                              |
   |                                              expected &str, found struct `std::string::String`
